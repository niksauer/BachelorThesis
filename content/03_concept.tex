...
\begin{itemize}
	...
	\item the threshold ($t$) to receive a penalty is set at 5 thrown away bottles
  	...
  	\item a consumer's penalty ($p_c$) is proportional to the number of bottles ($b$) he has thrown away during the lifetime of this system, meaning that $p_c = b \mod t \times v$ 
...
\begin{enumerate}[resume, label={\textbf{FR-\protect\twodigits{\theenumi}}},leftmargin=*]
	...
	\item As a retailer, I can report which one-way bottles a consumer has bought, so that the ownership of each bottle can be tracked.
...
\begin{enumerate}[resume, label={\textbf{FR-\protect\twodigits{\theenumi}}},leftmargin=*]  
	\item As a consumer, I can claim a reward for having purchased reusable bottles, so that I will be motivated to so in the future even though one-way packaging is lighter and more durable when compared to glass. 
\end{enumerate}
