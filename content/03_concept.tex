...
In accordance to \ref{sec:incentivizedOverview} and \ref{sec:rules}, the following functional requirements are defined as the bare minimum of features any implementation must provide for it to be used as the accounting backend of an incentivized deposit-refund system. These requirements are expressed from the perspective of an end-user (\textit{user story}) who hopes to achieve different goals by using the system and are sorted by the various roles encountered (comp.~\autoref{fig:useCaseOverview}).
